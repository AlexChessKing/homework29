...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        HttpGet request = new HttpGet(SERVER); // создаем объект запроса

        CloseableHttpResponse response = httpClient.execute(request); // отправляем запрос

        ObjectMapper mapper = new ObjectMapper();
        List<Cat> posts = mapper.readValue(response.getEntity().getContent(), new TypeReference<>() {});

        // фильтруем список и выводим результат на экран
